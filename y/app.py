...
def idofuser(userh):
    ...
    cur.execute("select name,userhandle,doj,verified,dp,background,bio from users where userhandle=%s",(user,))
    data=cur.fetchall()

    if data:
        lis=[]
        for n,uh,doj,ver,dp,bg,bio in data:
            n=str(n)
            uh=str(uh)
            doj=str(doj)
            v=str(ver[0])
            ...
            lis.append({"n":n,"uh":uh,"doj":doj,"v":vv,"dp":dp,"bg":bg,"bio":bio})
    ...
    try:
        if da is not  None:
            ...
            for n,uh,dt,cont,v,img,dp in da:
                ...
                cur.execute("select count(post) from likes where post=%s",(cont,))
                dat=cur.fetchall()
                dat=str(dat[0])
                dat=str(dat[1:len(dat)])
                dat = dat.replace(",)", "")
                ...
                l.append({"n": n, "uh": uh, "dt": dt, "cont": cont, "vv": vv, "img": image_data,"dp":dp,"lik":dat})
    ...
    return render_template("profile.html",lis=lis,l=l,er=er)
...
@app.route("/liking a post/<post>",methods=["POST","GET"])
def like(post):
    post=str(post)
    co=request.cookies.get("userid")
    abc=str(co)

    cur.execute("select userhandle from users where cookies=%s or userid=%s",(abc,abc,))
    us=cur.fetchone()
    us=str(us[0])
    
    cur.execute("select post from likes where userhandle=%s and post=%s",(us,post,))
    islik=cur.fetchall()

    if islik:
        cur.execute("delete from likes where post=%s and userhandle=%s",(post,us,))
    else:
        cur.execute("insert into likes (post,userhandle) values(%s,%s)",(post,us))
        db.commit()
        pass
...
def home():
    ...
    if data:
        ...
        for n,uh,dt,c,v,img,dp in data:
            ...
            cur.execute("select count(post) from likes where post=%s",(c,))
            lik=cur.fetchall()

            #<a href="{{url_for("like",post=i["c"])}}"><img src="https://i.pinimg.com/originals/a9/21/51/a92151c6cd493ade929a2a107e834d4a.gif" width="50" height="50" >{{i["lik"]}}</a>

            lik=str(lik[0])
            
            lik=str(lik[1:len(lik)])
            ...
            lis.append({"n": n, "uh": uh, "dt": dt, "c": c, "vv": vv, "img": image_data,"dp":dp,"lik":lik})
...
def set():
    ...
    cur.execute("select userhandle from users where cookies=%s or userid=%s",(cookies,cookies,))
    ab=cur.fetchone()
    us=str(ab[0])

    cur.execute("select verified from users where userhandle=%s",(us,))
    ver=cur.fetchone()
    ver=str(ver[0])

    cur.execute("select dp,userhandle,name from users where userhandle=%s",(us,))
    data=cur.fetchall()
    for dp,uh,un in data:
        dp=str(dp)
        uh=str(uh)
        un=str(un)

        lis=[]
        lis.append({"dp":dp,"uh":uh,"un":un})
    ...
    if ver=="t":
        ...
        return render_template("settings.html",lis=lis)
