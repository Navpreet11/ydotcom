...
def home():
    ...
    cur.execute("select name,userhandle,date,content,verification,image from posts")
    data=cur.fetchall()

    if data:
        lis=[]
        for n,uh,dt,c,v,img in data:
            ...
            lis.append({"n": n, "uh": uh, "dt": dt, "c": c, "vv": vv, "img": image_data})
...
def signup():
    ...
    if request.method=="POST":
        ...
        if abc:
            ...
            data=cur.execute("insert into users (name,userhandle,doj,password) values(%s,%s,%s,%s)",(n,h,d,p))
            db.commit()
...
def profile():
    ...
    cur.execute("select name,userhandle,doj,verified from users where userhandle=%s",(user,))
    data=cur.fetchall()

    if data:
        lis=[]
        for n,uh,doj,ver in data:
            n=str(n)
            uh=str(uh)
            doj=str(doj)
            v=str(ver[0])
            ...
            lis.append({"n":n,"uh":uh,"doj":doj,"v":vv})


    cur.execute("select name,userhandle,date,content,verification,image from posts where userhandle=%s",(user,))
    da=cur.fetchall()

    if da:
        l=[]
        for n,uh,dt,cont,v,img in da:
            ...
            l.append({"n": n, "uh": uh, "dt": dt, "cont": cont, "vv": vv, "img": image_data})
